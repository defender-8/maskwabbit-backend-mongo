...
exports.get = async (req, res, next) => {
  try {
    if (Object.keys(req.query).length !== 0) {
      ...
      const dataArr = await Category.find(conditions)
        .sort(sorter)
      ...
      const dataArr = await Category.find();
...
exports.put = async (req, res, next) => {
  ...
  try {
    ...
    const dataSingleUpdated = await Category.findById(id);
